...
4. ブレークポイントを設定する場所を考える<br>
   ブレークポイントを設定する箇所については、ある程度経験が必要です。

   今回は経験がなくてもブレークポイントの設定箇所についておおよそのあたりをつける方法を紹介します。
   以下の2つを守ってください。

   - エラーが出た箇所より先の行にブレークポイントを設定しない<br>
      これは単純にエラーがでた箇所より先の行にブレークポイントを設定しても、ブレークポイントでプログラムが止まらないからです

   - エラー文にあるスタックトレースを見る<br>
      スタックトレースとは、エラーが発生した箇所までに呼び出された関数やメソッドの一覧のことです。
      スタックトレースには、エラーが発生した箇所のファイル名と行番号が表示されます。

      この情報を元に、エラーが発生した箇所のファイルを開き、ブレークポイントを設定します。<br><br>
      先ほどのこのエラー文のスタックトレースは以下の通りです。
       1. `dbdata.php`の41行目でエラーが発生している
       2. `dbphp.php`の39行目で`DbData`クラスの`exec`メソッドを呼び出している
       3. `obj_insert.php`の15行目で、`DbPhp`クラスの`insertPerson`メソッドを15行目で呼び出している

      つまり、ブレークポイントを設定する箇所は、スタックトレースより先の行でなければOKなので、以下の3つを目安に設定してましょう。
      おすすめは、以下の直前です。
      - `dbdata.php`のエラーが発生した41行目
      - `dbphp.php`の39行目
   ...
   ```tip
   エラーが発生しない不具合あるので、上記のブレークポイント設定が通用しない場合もあります。
   その場合は、やはり経験が必要になります。
...
6. ブレークポイントが設定されたら、デバッガ起動の際に開かれたブラウザで、`obj_insert.php`のリンクをクリックする<br>
    ![](./images/breakpoint2_4_2.png)
